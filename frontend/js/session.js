...
function getSession() {
    try {
        ...
        console.error("Failed to read CareerForge session:", error);
        return structuredClone(DEFAULT_SESSION);
    }
}
...
function saveSession(session) {
    try {
        ...
        console.error("Failed to save CareerForge session:", error);
    }
}
...
function setInterviewQuestions(questions, sessionId = null) {
    const session = getSession();

    session.interview.questions = questions || [];
    session.interview.sessionId = sessionId;
    session.interview.currentQuestionIndex = 0;
    session.interview.answers = [];
    session.interview.report = null;
...
function saveInterviewAnswer(answer) {
    ...
    session.interview.answers.push(answer);
...
function setInterviewReport(report) {
    ...
    session.interview.report = report;
...
console.log(window.CareerForgeSession)